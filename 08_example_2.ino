...
#define INTERVAL 250 // sampling interval (unit: ms)
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    bright = (200.0 - dist_raw) * 2.55;
    analogWrite(PIN_LED, bright);
  }
  else if (200.0 < dist_raw && dist_raw < dist_max) {
    bright = (dist_raw - 300.0) * 2.55;
    analogWrite(PIN_LED, bright);
  }
  else {
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) reading = pre_reading; // return pre_reading when out of range.
  pre_reading = reading;
  return reading;
